    ...
    <div className="space-y-3">
            ...
            {resets.map((r) => {
              ...
              return (
                <div key={r.id} className="flex items-center justify-between gap-2">
                  <span className="text-white/80">{user?.email} · {fmtDate(r.created_at)}</span>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users..." className="w-full pl-9 pr-3 py-2 bg-white/5 border border-white/15 rounded-lg text-sm text-white outline-none focus:border-white" />
        </div>
            ...
            <tbody>
              {filtered.map((u) => (
                <tr key={u.id} className="border-t border-white/5 hover:bg-white/5">
                  ...
                  <td className="p-3 hidden md:table-cell text-white/60 text-xs">{u.phone || '—'}</td>
                  <td className="p-3">
                    <button onClick={() => toggleAdmin(u)} className={`text-xs px-2 py-0.5 rounded uppercase font-semibold ${u.role === 'admin' ? 'bg-white text-black' : 'bg-white/10 text-white/60'}`}>
                      {u.role}
                    </button>
                  </td>
                  ...
                  <td className="p-3 text-right">
                    <div className="inline-flex gap-1">
                      <button onClick={() => { setAdjustUser(u); setAdjustAmount('10'); }} className="p-1.5 rounded hover:bg-white/10" title="Adjust balance">
                        <Plus className="w-4 h-4" />
                      </button>
                      <button onClick={() => resetPassword(u)} className="p-1.5 rounded hover:bg-white/10" title="Reset password">
      ...
      {adjustUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={() => setAdjustUser(null)}>
          <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md bg-black border border-white/20 rounded-2xl p-6">
            ...
            <label className="block text-xs text-white/60 mb-1 uppercase">Amount (USD)</label>
            <input value={adjustAmount} onChange={(e) => setAdjustAmount(e.target.value)} type="number" className="w-full px-3 py-2 bg-white/5 border border-white/15 rounded-lg text-white outline-none mb-3" />
            <label className="block text-xs text-white/60 mb-1 uppercase">Note</label>
            <input value={adjustNote} onChange={(e) => setAdjustNote(e.target.value)} placeholder="Reason..." className="w-full px-3 py-2 bg-white/5 border border-white/15 rounded-lg text-white outline-none mb-4" />
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => adjust(-1)} className="py-3 border border-white/20 rounded-lg font-semibold flex items-center justify-center gap-2 hover:bg-white/10">
                <Minus className="w-4 h-4" /> Deduct
              </button>
              <button onClick={() => adjust(1)} className="py-3 bg-white text-black rounded-lg font-semibold flex items-center justify-center gap-2">
